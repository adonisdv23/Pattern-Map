...
- mobile Home: `37b0fcfcde15e3f1837c645085337041ea754e796f98179554bda2e0f890eedc`;
- desktop Map: `cefa9111c99f03574f140d1f92080ee7c8d76f5c4411da540070470f3cbdf1ea`.
...
| 390 x 844 | Home | `scrollWidth=390`; Read the idea, Explore the map, Apply it, and More are all computed visible; doors stack; no horizontal clipping. |
| 1440 x 1000 | Explore | F1-F6 cards visible in document order; six focus controls present; F1/F2 lead with ordinary-language summaries; focus changes emphasis without hiding other cards. |
...
On Explore, the initial status reads: `All six families are visible. Focus controls add emphasis; they never hide essential meaning.` Activating the F1 focus control changes it to: `F1 is focused. The other families remain visible for comparison.` The button exposes `aria-pressed="true"` in that state. The post-revision computed focus treatment is a 3px dark outline plus a 6px light outer ring, providing a visible pair across light and dark surfaces.
...
- `pdfinfo`: six letter-sized pages, unencrypted, no JavaScript, no form fields, and `Tagged: no`.
...
- The PDF now states on page 1 and page 6 that it is an untagged visual owner-review companion and directs assistive-technology readers to the semantic standalone HTML. It is not represented as the accessibility route.
...
3. A supported screen-reader pass remains a manual owner check. The standalone HTML, not the untagged PDF, is the semantic route.
4. Visual QA demonstrates implementation rendering and does not establish reader comprehension, effectiveness, persuasion, model quality, or research outcomes.
