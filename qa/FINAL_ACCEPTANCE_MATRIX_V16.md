...
the checkpoint register in `handoff/BRANCH_AND_PR_STATE.md`. The current evidence index
...
| A13 — responsive, keyboard, print, accessibility | **PARTIAL — OWNER CONFIRMATION REQUIRED** | Automated/static evidence passes narrow/medium/wide Map contracts, 54 Apply combinations, semantic landmarks/headings/names, visible mobile principal routes, persistent rail/mobile guide, scoped Read/Guided progress, contrast, focus styles, reduced motion, forced colors, no-script, print hooks, and six current PDF renders. Live checks cover default, 390, exact 821, and 1024 widths with no horizontal overflow; the 390-pixel term helper remains in flow. The standalone has one h1, 339 unique IDs, ten named sections, one `All routes` rail/guide/frame, complete fragments, and no false current route. | A physical end-to-end Tab/keyboard traversal, supported screen-reader review, real 200% browser/OS zoom, and browser print preview remain open. The PDF is intentionally an untagged visual companion; standalone HTML is the semantic accessibility route. Earlier `a319794` screenshots are historical and superseded for current Map/Apply semantics. |
