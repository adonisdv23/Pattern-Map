...
| C16-012 | Signal Foundry and the two neutral cases illustrate how the six families and records can be applied | `ILLUSTRATIVE_CASE` | Case headers, fixtures, cost/stop boundaries, and route mappings | None as validation; cases are deliberately local and invented/bounded | Runtime behavior, implemented integration, private-data finding, product validation, or permission to act |
| C16-013 | The Echo Problem is a separate EP v0.1 successor whose source checkpoint is historical v15.2; its protocol and deterministic harness are preserved and it is unrun with no results | `ARTIFACT_INTEGRITY` + project-status record | EP README/status/version history; 239-file accession; 82-file curated verifier; 15 deterministic tests | Separate Echo route only | Evidence that origin accounting improves a model, a completed study, or the definition of v16 |
| C16-014 | DL-PLAYBOOK-01 could later compare the operational playbook with ordinary prompting under matched total budgets on usefulness, supported novelty, evidence diversity, missing-perspective detection, and correction effort | `FUTURE_EMPIRICAL_QUESTION` | Broader research agenda and matched-budget protocol candidate v0.1 | Targeted prior art constrains design; no result source exists | A result, selected model/provider, preregistration, analysis-ready protocol, sample-size claim, or authorization to run |
