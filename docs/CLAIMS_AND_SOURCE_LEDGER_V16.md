...
This ledger records what the active v16 artifacts may say, the strongest basis
currently available for saying it, and the upgrade that is not allowed. It is
not a score of truth and does not turn owner intent, a historical artifact, a
validator, or an advisory review into empirical evidence.

The source authority order in `SOURCE_AUTHORITY_AND_LINEAGE.md` governs when
records conflict. The optional public route in
`manuscript/SOURCES_AND_RESEARCH_ROUTE.md` remains targeted rather than
exhaustive. External links must be rechecked before any later-authorized public
release.
...
| C16-002 | Pattern Recognition is the discipline of improving those choices; the Discrimination Layer names the explicit, inspectable, cost-bounded, and correctable responsibility before generation | `OWNER_PREMISE` + `DESIGN_HYPOTHESIS` | Owner intent; thesis contract; essay; public abstract | Adjacent provenance, decision-support, and human-AI control work | A newly discovered neural layer, protected-class classification, human-like discernment, or mandatory architecture |
...
| C16-005 | Source role, exact claim support, relevance, recurrence, origin, provenance, authority, and permission are different judgments | `CONCEPTUAL_SYNTHESIS` + `DESIGN_HYPOTHESIS` | Essay source section; F2 specification; glossary and evidence records | W3C PROV-O, claim provenance, evidence-synthesis, and human-control sources | Provenance is correctness; authority is universal trust; recurrence is independent corroboration; access is permission |
...
| C16-014 | DL-PLAYBOOK-01 could later compare the operational playbook with an ordinary baseline and a generic-diligence comparator under matched resources, but mechanism-isolated questions must precede an omnibus interpretation | `FUTURE_EMPIRICAL_QUESTION` | Broader research agenda, narrow-wedge decision memo, and matched-budget protocol candidate v0.1 | Current agent-evaluation and deep-research work constrains baselines, resource accounting, ablations, and reproducibility; no result source exists | A result, selected model/provider/corpus, preregistration, analysis-ready protocol, sample-size claim, or authorization to run |
...
| C16-016 | The component practices around context curation, retrieval and selection, source reliability and redundancy, adaptive acquisition and stopping, memory, attribution, correction, and deep-research evaluation have substantial established and active prior work; v16's bounded contribution is an authored human-governed design/governance synthesis and testable agenda | `CONCEPTUAL_SYNTHESIS` + `REJECTED_OR_NARROWED` | Owner intent; thesis contract; targeted 2024–2026 source route; research agenda | Primary and official route checked 2026-08-30; targeted, not systematic or exhaustive | A novel mechanism, exhaustive taxonomy, newly discovered technical layer, empty-field claim, effectiveness result, or proof that holding the practices together improves decisions |
| C16-017 | Two narrow future-study candidates—a composite claim-scoped influence receipt and an orthogonal observation/process/access/permission/currency boundary record—can be compared as design decisions before any provider, model, corpus, participant, or run is selected | `FUTURE_EMPIRICAL_QUESTION` | Narrow-wedge research decision memo v0.1 | Adjacent attribution, gap-judging, memory, and evaluation work establishes overlap and unresolved novelty; Candidate A needs a study-mode choice and predeclared ablations, while Candidate B needs separate task/world keys and trace-derived run states | A chosen first paper, a ready protocol, a registered hypothesis, an acquired corpus, a participant plan, or authorization to execute |

## Rejected or narrowed formulations

The following formulations are not allowed in current v16 public copy unless a
later exact owner instruction and adequate evidence change the record:
...
- the Discrimination Layer is one required software component or model
  architecture;
- a custom model is inherently the deepest or most defensible path;
- the playbook, Signal Foundry case, Echo harness, validators, advisory audits,
  or polished site prove that the framework works;
- non-experts or agents can replace expert judgment; or
- an unrun protocol, fixture, power scaffold, or future outcome category is a
  research result.
- same-original news reproduction labels establish claim support, truth, or
  independence; different reproduction clusters are automatically independent;
- every measured origin-rate estimate is necessarily a lower bound; or
- nobody has measured retrieval redundancy/dependence and the measurement
  paper therefore has an uncontested novelty claim.
- Pattern Map, the Discrimination Layer, or its six-family arrangement is the
  first, only, or proven way to govern pre-generation context choices;
- context engineering, adaptive retrieval/stopping, memory, attribution,
  source diversity, or correction was invented by this project; or
- a citation, influence estimate, retrieved-document attribution, benchmark
  label, or accepted human correction establishes truth by itself.

## Maintenance rule

Every material new public claim must receive an ID, strongest actual status,
canonical basis, and prohibited upgrade before integration. A source may
constrain or contextualize a claim without validating the six-family
arrangement. Research-driven edits may narrow a claim; they may not silently
replace the owner-approved thesis. Later evidence changes receive a new ledger
entry or explicit revision record rather than retroactive inflation of the
current basis.
