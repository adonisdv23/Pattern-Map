...
const termHelp = {
  ...
  "upstream-choices": {
    ...
    detail: "Generation inherits these choices. A polished answer cannot make an omitted perspective or unmade comparison inspectable after the fact.",
...
const familyPublicCopy = {
  ...
  F2: {
    purpose: "Weigh what the source can support without turning its pathway, reputation, or repetition into proof.",
    boundary: "Keep source role, track record, authority, support, recurrence, origin, relevance, provenance, and permission separate.",
...
const renderDoorCard = (id, ctx) => {
  ...
  const promise = isPublic(ctx) && id === "read"
  ...
  return `<a class="door-card door-${id}" href="${routeHref(ctx, id, id === "read" ? "read-idea" : id)}">
    ...
    <span class="door-promise">${escapeHtml(promise)}</span>
...
const renderPublicationMetadata = ({ title, intro, ctx, active }) => {
  ...
  const shareTitle = active === "home" ? homeShareTitle : `${title} — Pattern Map v16`;
  const description = active === "home" ? homeShareDescription : (intro || contentInterface.first_screen.standfirst);
  ...
  const socialImageAlt = releaseMetadataEnabled ? publicationConfig.social_image_alt : "";
  return [
    ...
    socialImage ? `<meta property="og:image" content="${escapeAttribute(socialImage)}"><meta property="og:image:alt" content="${escapeAttribute(socialImageAlt)}"><meta name="twitter:image" content="${escapeAttribute(socialImage)}"><meta name="twitter:image:alt" content="${escapeAttribute(socialImageAlt)}">` : "",
...
const renderFamilyCard = (family, ctx) => {
  ...
  return `<article id="family-${family.id}" class="family-card ${colorClass}" data-family-card="${escapeAttribute(family.id)}">
    ...
    <div class="family-columns"><div><h4>How it works</h4><p>${inlineMarkdown(publicCopy.mechanism, ctx)}</p></div><div><h4>Boundary</h4><p>${inlineMarkdown(publicCopy.boundary ?? family.boundaries[0], ctx)}</p></div></div>
...
const renderRoot = (ctx) => {
  ...
  <section class="hero" id="top">
    <div class="hero-copy">
      <p class="eyebrow hero-eyebrow">${isPublic(ctx) ? "PATTERN RECOGNITION / BEFORE GENERATION" : "PATTERN RECOGNITION / THE DISCRIMINATION LAYER"}</p>
  ...
  ${isPublic(ctx) ? `<section class="home-section home-short home-short-bridge" aria-labelledby="short-entry-heading"><div class="section-heading"><p class="eyebrow">A CUMULATIVE 60–90 SECOND ENTRY</p><h2 id="short-entry-heading">Carry the broad idea into the essay.</h2><p>The quick example above shows one decision changing before prose. The short Read entry names the complete six-family proposition, its human boundary, and why ordinary work can stay ordinary—without repeating the release example here.</p></div><a class="text-link" href="${routeHref(ctx, "read", "read-quick")}">Read the cumulative 60–90-second version <span aria-hidden="true">→</span></a></section>` : `<section class="home-section home-short" aria-labelledby="short-entry-heading"><div class="section-heading"><p class="eyebrow">A CUMULATIVE 60–90 SECOND ENTRY</p><h2 id="short-entry-heading">The idea before the machinery.</h2><p>Start here if you want the broad proposition in one sitting. The longer piece remains a human thought piece, not a protocol preamble.</p></div><div class="short-entry reading-column">${short}</div><a class="text-link" href="${routeHref(ctx, "read", "read-idea")}">Continue into the complete thought piece <span aria-hidden="true">→</span></a></section>`}
...
const renderImplementationLevels = (ctx) => `
  <section class="implementation-section" aria-labelledby="levels-heading">
    <div class="section-heading"><p class="eyebrow">FOUR PROPORTIONATE CHOICES</p><h2 id="levels-heading">Choose the smallest route that fits the decision.</h2><p>No provider, model, database, graph, or custom service is required. Ordinary remains available only when every Stage 0 condition immediately above holds.</p></div>
    <div class="implementation-grid">
      <article class="implementation-card level-ordinary"><span class="level-pill">ordinary</span><h3>Do less when the task is simple.</h3><p>A format conversion, translation, rewrite, summary, or other supplied-input transformation may use an ordinary record only when every Stage 0 condition holds.</p><p class="card-boundary"><strong>Do not use the full framework</strong> when the record would cost more than the consequence of being wrong.</p></article>
...
const renderRouteStudio = (ctx) => `
  <section class="route-studio-section" id="route-studio" aria-labelledby="route-studio-heading">
    <div class="route-brief" aria-label="How to use the Apply route"><span><strong>What this is</strong> A local planning surface.</span><span><strong>What you can do</strong> End ordinary work at Stage 0 or size one layered route.</span><span><strong>What it cannot do</strong> Record a run, outcome, stop event, permission, or human decision.</span></div>
    <div class="section-heading"><p class="eyebrow">LOCAL ROUTE STUDIO / NO PROVIDER</p><h2 id="route-studio-heading">Build a recommendation, not a fictional receipt.</h2><p>Supplied material does not automatically make a task ordinary. Test the exact Stage 0 choice below; if any disqualifier is present, choose the smallest layered conditions. The browser can recommend a level, next action, required gate, ${renderTerm("planned-stop-condition", "apply-stop", ctx)}, and learning option. It does not grant permission, execute work, observe an outcome, or decide for anyone. ${renderTerm("human-authority", "apply-authority", ctx)} remains separate.</p></div>
    ...
    <div class="route-studio-layout">
      <form class="route-studio-form" data-route-studio>
        <fieldset><legend>00 · Is a layered evidence decision required?</legend><label><input type="radio" name="evidenceSelection" value="none" checked> No — already-permitted supplied-material transformation only <small>reversible, with no material claim judgment, comparison, selection or withholding, permission resolution, memory reuse, acquisition, human action gate, or consequential external influence</small></label><label><input type="radio" name="evidenceSelection" value="needed"> Yes — a Stage 0 disqualifier is present <small>use the smallest warranted evidence route</small></label></fieldset>
        <p class="stage0-status" id="stage0-dependent-status" data-stage0-status role="status">Consequence, uncertainty, capacity, permission, and the human action gate are not applicable while Stage 0 remains an already-permitted supplied-material transformation.</p>
        ...
        <fieldset data-stage0-dependent="budget" aria-describedby="stage0-dependent-status" disabled><legend>03 · Approved capacity</legend><label><input type="radio" name="budget" value="quick" checked> Quick <small>short bounded pass</small></label><label><input type="radio" name="budget" value="bounded"> Bounded <small>enough capacity for a reproducible record</small></label><label><input type="radio" name="budget" value="substantial"> Substantial <small>capacity permits deeper work; it never justifies it by itself</small></label></fieldset>
        <fieldset data-stage0-dependent="permission" aria-describedby="stage0-dependent-status" disabled><legend>04 · Permission state</legend><label><input type="radio" name="permission" value="AUTHORIZED" checked> <code>AUTHORIZED</code> <small>a scoped permission decision exists for the proposed use</small></label><label><input type="radio" name="permission" value="UNKNOWN"> <code>UNKNOWN</code> <small>permission has not been established; do not infer it</small></label><label><input type="radio" name="permission" value="NOT_AUTHORIZED"> <code>NOT_AUTHORIZED</code> <small>permission is absent or denied</small></label><label><input type="radio" name="permission" value="REVOKED"> <code>REVOKED</code> <small>a prior authorization no longer applies</small></label></fieldset>
        <fieldset data-stage0-dependent="humanActionGate" aria-describedby="stage0-dependent-status" disabled><legend>05 · Separate human action gate</legend><label><input type="radio" name="humanActionGate" value="NOT_REQUIRED" checked> <code>NOT_REQUIRED</code> <small>no separate human approval is needed for the bounded planned output</small></label><label><input type="radio" name="humanActionGate" value="REQUIRED"> <code>REQUIRED</code> <small>the system may prepare; the named person must decide before consequential use or action</small></label></fieldset>
      ...
      <aside class="route-recommendation-card" data-route-recommendation>
        ...
        <h3 data-recommendation-title>End ordinary work with four fields.</h3>
        <p data-recommendation-summary>Stage 0 found no evidence-selection work. Perform the reversible supplied-material transformation, write the four-field ordinary record, and do not manufacture a layered workflow.</p>
        <section aria-labelledby="recommendation-heading"><h4 id="recommendation-heading">Recommended plan</h4><dl class="recommendation-facts"><div><dt>Recommended action</dt><dd data-recommendation-action>ORDINARY_RECORD</dd></div><div><dt>Permission state</dt><dd data-recommendation-permission>NOT_APPLICABLE</dd></div><div><dt>Human action gate</dt><dd data-recommendation-human-gate>NOT_APPLICABLE</dd></div><div><dt>Capacity fit</dt><dd data-recommendation-capacity>NOT_APPLICABLE</dd></div><div><dt>Required gate</dt><dd data-recommendation-gate>Use Stage 0 only when the supplied-material transformation is already permitted, reversible, and needs no material claim judgment, comparison, selection or withholding, permission resolution, memory reuse, acquisition, human action gate, or consequential external influence.</dd></div><div><dt>Planned stop condition</dt><dd data-recommendation-stop>Return only the supplied scope, material assumptions, unchecked boundaries, and output; then stop.</dd></div><div><dt>Learning option</dt><dd data-recommendation-learning>LEARNING_NOT_APPLICABLE — this ordinary record creates no route, stop event, outcome, or learning event.</dd></div></dl></section>
        <p class="recommendation-status" data-recommendation-status role="status" aria-live="polite" aria-atomic="true">Four-field ordinary record recommended; no layered route, execution, or human decision has been recorded.</p>
    ...
    <details class="static-route-equivalent" open${isPublic(ctx) ? " data-progressive-static-guide" : ""}><summary>Static decision guide: complete no-script equivalent</summary><div class="static-route-body"><p><strong>Stage 0 comes first and is terminal:</strong> use it only for an already-permitted, reversible supplied-material transformation that needs no material claim judgment, comparison, selection or withholding, permission resolution, memory reuse, acquisition, human action gate, or consequential external influence. Return only <strong>supplied scope, material assumptions, unchecked boundaries, and output</strong>, then stop. All layered fields are <strong>not applicable</strong>. If any of those decisions is required, use the smallest layered path.</p><p>For layered work, permission and the human action gate stay separate. Preserve <code>AUTHORIZED</code>, <code>UNKNOWN</code>, <code>NOT_AUTHORIZED</code>, and <code>REVOKED</code> exactly. Approved capacity limits what may be attempted; it never makes Advanced appropriate by itself. The table expresses planning recommendations only. It does not claim that work ran, a resource boundary was reached, an outcome was observed, learning occurred, permission was granted, or a person made a decision.</p><p><strong>When conditions overlap, resolve them in this order:</strong> (1) unresolved or blocked permission determines <code>ESCALATE</code> or <code>HOLD</code>; (2) an authorized route with a required human action gate remains <code>HOLD</code>; (3) an authorized route without that gate but with insufficient capacity becomes <code>CLARIFY</code> and <code>NARROW_OR_ESCALATE</code>; (4) only then use the base level action. Keep every lower-priority state visible even when an earlier gate controls the action.</p><div class="table-wrap"><table><caption>Stage 0 and proportionate planning choices</caption><thead><tr><th scope="col">Choice</th><th scope="col">Fit</th><th scope="col">Recommended action</th><th scope="col">Required gate</th><th scope="col">Planned stop condition</th></tr></thead><tbody><tr><th scope="row">ordinary</th><td>No layered evidence decision is required: Stage 0 is terminal for an already-permitted supplied-material transformation; all layered fields are not applicable</td><td><code>ORDINARY_RECORD</code></td><td>no permission resolution, evidence judgment, human action gate, or consequential external influence is required</td><td>return the four fields, then stop</td></tr><tr><th scope="row">lightweight</th><td>A Stage 0 disqualifier is present: one bounded comparison or alternate route is warranted for reversible work with low or mixed uncertainty</td><td><code>ANSWER_PROVISIONALLY</code></td><td>human review before consequential use</td><td>one route + one challenge + stated resource limit</td></tr><tr><th scope="row">moderate</th><td>consequential or high-uncertainty evidence-selection work that does not meet every Advanced condition</td><td><code>COMPARE</code></td><td>named human review before consequential use</td><td>named comparison, critical gap, or approved resource boundary</td></tr><tr><th scope="row">advanced</th><td>consequential + high uncertainty + substantial approved capacity; queryable records and repeated review justify their cost</td><td><code>COMPARE</code></td><td>explicit permission + accountable review</td><td>approved resource boundary or blocking gap</td></tr><tr><th scope="row"><code>UNKNOWN</code> permission</th><td>permission has not been established</td><td><code>ESCALATE</code></td><td>authorized person or policy owner must establish scoped permission</td><td>preserve <code>UNKNOWN</code>; do not infer consent</td></tr><tr><th scope="row"><code>NOT_AUTHORIZED</code> permission</th><td>permission is absent or denied</td><td><code>HOLD</code></td><td>new explicit scoped authorization</td><td>keep material out of acquisition, use, disclosure, and influence</td></tr><tr><th scope="row"><code>REVOKED</code> permission</th><td>a prior authorization no longer applies</td><td><code>HOLD</code></td><td>new explicit scoped authorization; do not reuse the old one</td><td>keep prior material out of reuse, disclosure, and influence</td></tr><tr><th scope="row">human action gate <code>REQUIRED</code></th><td>permission may be authorized, but a separate person must approve consequential use or action</td><td><code>HOLD</code></td><td>named human decision</td><td>remain on hold until that action gate is satisfied</td></tr><tr><th scope="row">capacity mismatch</th><td>consequential + high uncertainty with only quick or bounded approved capacity</td><td><code>CLARIFY</code></td><td>narrow the decision or obtain a separately approved larger boundary</td><td>record <code>NARROW_OR_ESCALATE</code>; do not silently under-scope</td></tr></tbody></table></div><p class="static-plan"><strong>Observed state remains:</strong> execution <code>NOT_RUN</code> · stop outcome <code>NOT_TRIGGERED</code> · outcome <code>NOT_OBSERVED</code> · learning review <code>NOT_AVAILABLE</code> · human decision <code>NOT_RECORDED</code>.</p></div></details>
    <p class="studio-boundary capacity-boundary"><strong>Capacity mismatch:</strong> if consequence and uncertainty warrant deeper work but the selected quick or bounded capacity cannot support it, narrow the decision or obtain a separately approved larger boundary. Do not silently under-scope the work, and do not treat a larger budget as evidence that Advanced is warranted.</p>
  </section>`;
...
const renderGuided = (ctx) => {
  ...
  <section class="guided-route" id="guided" data-reading-route>
    <div class="route-brief${isPublic(ctx) ? " route-brief-public" : ""}" aria-label="How to use the Guided read">${isPublic(ctx) ? `<span><strong>Reading path</strong> One continuous route from the human problem to a bounded application.</span><span><strong>Time</strong> Approximately 8–12 minutes; editorial estimate only.</span>` : `<span><strong>What this is</strong> One continuous path through the publication.</span><span><strong>What it preserves</strong> Read, Map, and Apply remain separate doors.</span><span><strong>Time</strong> Approximately 8–12 minutes; editorial estimate only.</span>`}</div>
    ...
    <section class="guided-section" id="guided-apply" data-reading-section aria-labelledby="guided-apply-heading"><div class="section-heading"><p class="eyebrow">04 · PROPORTIONALITY</p><h2 id="guided-apply-heading">First ask whether evidence selection is even part of the task.</h2><p>Supplied material does not automatically make a task ordinary. Use the ordinary path only when the transformation is already permitted and reversible, with no material claim judgment, comparison, selection or withholding, permission resolution, memory reuse, acquisition, human action gate, or consequential external influence. Otherwise use the smallest layered route.</p></div>${renderImplementationLevels(ctx)}<p class="guided-transition"><a href="${routeHref(ctx, "apply", "route-studio")}">Build a local route recommendation <span aria-hidden="true">→</span></a></p></section>
    <section class="guided-section" id="guided-examples" data-reading-section aria-labelledby="guided-examples-heading"><div class="section-heading"><p class="eyebrow">05 · TWO SCALES</p><h2 id="guided-examples-heading">Ordinary work can stay ordinary. Consequential work earns more inspection.</h2></div><div class="guided-example-grid"><article><span class="level-pill level-ordinary">ordinary</span><h3>Reformat supplied prose exactly.</h3><p>Wrap the complete supplied text in a reversible format without changing, selecting, or omitting its content. Return the four-field record, then stop.</p></article><article><span class="level-pill level-moderate">consequential</span><h3>Assess a release with conflicting signals.</h3><p>Name the decision, permission, baseline, comparison, missing fields, main challenge, planned stop condition, and human review point before consequential use.</p></article><article><span class="level-pill level-light">relationship</span><h3>Nine reports, one source path.</h3><p>Record nine observations and one known origin without converting repetition into nine independent confirmations. The Echo project remains separate and unrun.</p></article></div><p class="guided-transition"><a href="${routeHref(ctx, "examples", "examples")}">Explore all worked examples and bounded cases <span aria-hidden="true">→</span></a></p></section>
...
const pageDefinitions = (ctx) => ({
  ...
  read: renderPage({ title: "Read the idea", eyebrow: "PRINCIPAL DOOR 01", intro: isPublic(ctx) ? "Read why choices made before generation shape what an answer can become." : "Continue the coffee conversation: why choices made before generation shape what an answer can become.", content: renderRead(ctx), ctx, active: "read", id: "read-page" }),
