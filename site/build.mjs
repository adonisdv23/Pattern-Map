...
const renderRouteStudio = (ctx) => `
  <section class="route-studio-section" id="route-studio" aria-labelledby="route-studio-heading">
    ...
    <div class="section-heading"><p class="eyebrow">LOCAL ROUTE STUDIO / NO PROVIDER</p><h2 id="route-studio-heading">Build a recommendation, not a fictional receipt.</h2><p>First decide whether the task only performs an already-permitted, reversible transformation of supplied material. If it does, return the four-field ordinary record and stop. If the system must judge, select, compare, preserve, acquire, or let material influence an answer, choose the layered conditions. The browser can recommend a level, next action, required gate, ${renderTerm("planned-stop-condition", "apply-stop", ctx)}, and learning option. It does not grant permission, execute work, observe an outcome, or decide for anyone. ${renderTerm("human-authority", "apply-authority", ctx)} remains separate.</p></div>
    ...
    <div class="route-studio-layout">
      <form class="route-studio-form" data-route-studio>
        <fieldset><legend>00 · Is a layered evidence decision required?</legend><label><input type="radio" name="evidenceSelection" value="none" checked> No — already-permitted supplied-material transformation only <small>no material claim judgment, comparison, selection, withholding, permission decision, memory reuse, acquisition, or external influence</small></label><label><input type="radio" name="evidenceSelection" value="needed"> Yes — material must be judged, selected, compared, preserved, acquired, or allowed to influence <small>use the smallest warranted evidence route</small></label></fieldset>
